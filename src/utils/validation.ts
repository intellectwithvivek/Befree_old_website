...
export const validateMerchantInfo = (merchant: Merchant) => {
  if (merchant.username && merchant.uid && merchant.name && merchant.district && merchant.country && merchant.division && merchant.lat && merchant.lng && merchant.state && merchant.formatted_address)
    return true;
  else return false;
};

export const validateIntializationMerchantInfo = (merchant: Merchant) => {
  if (merchant.type && merchant.type != 'Select' && merchant.place && merchant.district && merchant.division && merchant.country  && merchant.lat && merchant.lng && merchant.state && merchant.formatted_address && merchant.postal_code && merchant.place_id)
    return true;
  else return false;
};
