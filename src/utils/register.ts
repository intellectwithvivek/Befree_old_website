...
import { GoogleUser , Merchant, userCredentials } from '../@types/interfaces/merchant';
 

 export const createRegisteringUser = (additionalUserInfo:userCredentials,via:string):Merchant => {
...
export const createRegisteringUserViaAuthStateChange = (user:GoogleUser,via:string):Merchant => {
   const uid = user?.uid  
   const username = shorthash.unique(uid);
   let app_user={};
   if(via == VIA.PHONE)
