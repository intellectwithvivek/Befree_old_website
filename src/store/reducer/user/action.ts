...
import {
  createRegisteringUser,
  createRegisteringUserViaAuthStateChange,
} from "../../../utils/register";
...
export const registerUser =
  (userCredentials: any, via: string, with_auth?: string) =>
  async (dispatch: Dispatch) => {
    try {
      dispatch(setAppLoading(true));
      const user = with_auth
        ? createRegisteringUserViaAuthStateChange(userCredentials, via)
        : createRegisteringUser(userCredentials, via);
...
export const getMerchantInfo =
  (userData: Merchant) => async (dispatch: Dispatch) => {
    try {
      if (userData.username) {
        dispatch(setAppLoading(true));
        const { data } = await userDetailsService.getMerchantInfo(
          userData?.username
        );
...
export const initializeMerchantInfo =
  (userData: Merchant) => async (dispatch: Dispatch) => {
    try {
      if (userData.username) {
        dispatch(setAppLoading(true));
...
export const verifytheUser =
  (merchantInfo: Merchant) => async (dispatch: Dispatch) => {
    try {
      const response = await userDetailsService.verifyMerchantInfo(
        merchantInfo
      );
...
export const OTPverification =
  (verification: any, place_image: string, username: string) =>
  async (dispatch: Dispatch) => {
    try {
      if (verification) {
